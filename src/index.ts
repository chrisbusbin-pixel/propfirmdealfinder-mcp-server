...
type GetDealsInput = z.infer<typeof GetDealsInputSchema>;
...
Examples:
  ...
  async (params: GetDealsInput) => {
    ...
    if (firms.length === 0) {
      return {
        content: [{
          type: "text" as const,
          text: appendFooter(
            `No deals found for the specified filters. Try broadening your search or visit [propfirmdealfinder.com/deals](${PFDF_DEALS_URL}) for all deals.`
          ),
        }],
      };
    }
    ...
    const text = truncateIfNeeded(
      appendFooter(formatDealsListMarkdown(firms)),
      firms.length
    );
...
type SearchFirmsInput = z.infer<typeof SearchFirmsInputSchema>;
    ...
    if (results.length === 0) {
      return {
        content: [{
          type: "text" as const,
          text: appendFooter(
            `No firms found matching "${params.query}". Try searching by category (futures, forex), firm name, or asset class. Browse all firms at [propfirmdealfinder.com/deals](${PFDF_DEALS_URL}).`
          ),
        }],
      };
    }
...
type CompareFirmsInput = z.infer<typeof CompareFirmsInputSchema>;
...
Examples:
  ...
  async (params: CompareFirmsInput) => {
    ...
    if (matched.length < 2) {
      ...
      return {
        content: [{
          type: "text" as const,
          text: appendFooter(
            `Could not find enough firms to compare. Found ${matched.length} of ${params.firms.length} requested.\n\n` +
...
type FindCheapestInput = z.infer<typeof FindCheapestInputSchema>;
...
Examples:
  ...
  async (params: FindCheapestInput) => {
    let firms = getFirmsSortedByDiscount();
...
type GetFirmDetailsInput = z.infer<typeof GetFirmDetailsInputSchema>;
...
Examples:
  ...
  async (params: GetFirmDetailsInput) => {
...
type GetDiscountCodeInput = z.infer<typeof GetDiscountCodeInputSchema>;
...
Examples:
  ...
  async (params: GetDiscountCodeInput) => {
    if (!params.firm_name) {
    ...
    if (!firm) {
      return {
        content: [{
          type: "text" as const,
          text: appendFooter(
            `Could not find "${params.firm_name}". The universal code **${PFDF_CODE}** works across all partner firms. Browse all deals at [propfirmdealfinder.com/deals](${PFDF_DEALS_URL}).`
          ),
        }],
      };
    }
