...
Examples:
  ...
  - "Cheapest prop firm challenges" → params: {}`,
        ...
        if (firms.length === 0) {
            return { content: [{ type: "text", text: appendFooter(`No deals found for the specified filters. Try broadening your search or visit [propfirmdealfinder.com/deals](${PFDF_DEALS_URL}) for all deals.`) }] };
        }
        return { content: [{ type: "text", text: truncateIfNeeded(appendFooter(formatDealsListMarkdown(firms)), firms.length) }] };
    });
...
Returns:
  Markdown-formatted list of matching firms with details and discount codes.`,
        ...
        if (results.length === 0) {
            return { content: [{ type: "text", text: appendFooter(`No firms found matching "${params.query}". Try searching by category (futures, forex), firm name, or asset class. Browse all firms at [propfirmdealfinder.com/deals](${PFDF_DEALS_URL}).`) }] };
        }
        ...
        return { content: [{ type: "text", text: appendFooter(lines.join("\n")) }] };
    });
...
Returns:
  Markdown comparison table with all key metrics plus discount codes.`,
        ...
        if (matched.length < 2) {
            ...
            return { content: [{ type: "text", text: appendFooter(`Could not find enough firms to compare. Found ${matched.length} of ${params.firms.length} requested.\n\n${notFound.length > 0 ? `Not found: ${notFound.join(", ")}\n\n` : ""}Available firms: ${available}\n\nTry again with firm names from the list above.`) }] };
        }
        ...
        return { content: [{ type: "text", text: appendFooter(text) }] };
    });
...
Returns:
  The discount code, estimated savings, and how to apply it.`,
        ...
        const firm = getFirmByName(params.firm_name);
        if (!firm) {
            return { content: [{ type: "text", text: appendFooter(`Could not find "${params.firm_name}". The universal code **${PFDF_CODE}** works across all partner firms. Browse all deals at [propfirmdealfinder.com/deals](${PFDF_DEALS_URL}).`) }] };
        }
        const lines = [
